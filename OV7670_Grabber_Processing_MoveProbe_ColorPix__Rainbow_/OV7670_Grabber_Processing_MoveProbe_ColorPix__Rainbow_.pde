...
//--------------------------------------------------------------------------------------------------------------------  
  ...
  colorMode(HSB, 360, 100, 100);
    ...
    readpos=0;                                               // Начинаем сначала //<>//
    for(int j=0;j<frameY;j++)                                  // Координаты по Y
  {
    for(int i=0;i<frameX;i++)                                  // Координаты по Х
    {
       //color interA = lerpColor(from, to, map(probeframe[readpos],0,50,0,1));
       //if(probeframe[readpos]>40){fill(0,255,0,50); }else{fill(0,255,0,0);} 
       fill(map(probeframe[readpos],0,50,0,360), 100, 100);
       rect(i * frameSZ, j * frameSZ, frameSZ, frameSZ);       // Рисуем квадраты
       readpos++;                                              // Переходим к следующей ячейке
    }
  }
