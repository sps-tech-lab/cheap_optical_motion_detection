...
byte    frame[NUM_PIXS+11];               // Буффер кадра
byte    ter[225];
...
//  pixel_grab(frame, NUM_PIXS);          // Считываем массив кадра
    pixel_and_params_grab(frame);         // Считываем массив кадра и параметры сенсора
  
  Serial.write(0xFF);
  
  for (int i = 0;i <NUM_PIXS+11; i++)           // Шлем данные "постепенно"
  {
...
/*
  ...
  for (int i = 0;i <225; i++)            // Остальное закомментить
  {
    ter[i] = i;
  }
