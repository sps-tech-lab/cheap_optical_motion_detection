...
void MoveProbe()
{
  ...
  for(int by=0; by<10; by++)              
  {
    ...
    {
      for(int biy=0; biy<12; biy++)
      {
        for(int bix=0; bix<16; bix++)                
        {
          probebloks[by*10+bx]+=probeframe[biy*160+by*10+bix*bx];
        }
      }
      //probebloks[by*10+bx]=(by*10+bx)*2;                 // Получаем среднее арифметическое
      probebloks[by*10+bx]=(probebloks[by*10+bx]/192)*4;
    }
  }
